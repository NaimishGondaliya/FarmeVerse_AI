...
export const ExpertProfile = () => {
    ...
    const handleProfileSave = async (e) => {
        ...
        if (!editForm.qualification.trim()) {
            triggerProfileMsg('error', lang === 'GUJ' ? 'લાયકાત ફરજિયાત છે.' : 'Qualification is required.')
            return
        }
        const expNum = Number(editForm.experience)
        if (isNaN(expNum) || expNum < 0 || expNum > 80) {
            triggerProfileMsg('error', lang === 'GUJ' ? 'અનુભવ 0-80 વર્ષની વચ્ચે હોવો જરૂરી છે.' : 'Experience must be between 0 and 80 years.')
            return
        }
        if (editForm.phone && !/^\d{10}$/.test(editForm.phone)) {
            triggerProfileMsg('error', lang === 'GUJ' ? 'મોબાઈલ નંબર ૧૦ અંકનો હોવો જોઈએ.' : 'Phone must be exactly 10 digits.')
            return
        }
        ...
        try {
            ...
            setProfile(updated)
            setIsEditing(false)
            triggerProfileMsg('success', lang === 'GUJ' ? 'પ્રોફાઇલ સફળતાપૂર્વક અપડેટ થયું!' : 'Profile updated successfully!')
        } catch (err) {
            console.error(err)
    ...
    const handlePasswordSubmit = async (e) => {
        ...
        if (passwords.newPassword !== passwords.confirmPassword) {
            setPwdMsg({ type: 'error', text: lang === 'GUJ' ? 'નવો પાસવર્ડ મેળ ખાતો નથી.' : 'New passwords do not match.' })
            return
        }
        if (passwords.newPassword.length < 6) {
            setPwdMsg({
                type: 'error',
                text: lang === 'GUJ' ? 'પાસવર્ડ ઓછામાં ઓછો ૬ અક્ષરનો હોવો જોઈએ.' : 'Password must be at least 6 characters.'
            })
            return
        }
        ...
        try {
            ...
            setPwdMsg({
                type: 'success',
                text: lang === 'GUJ' ? 'પાસવર્ડ સફળતાપૂર્વક અપડેટ થયો!' : 'Password updated successfully!'
            })
            setPasswords({ currentPassword: '', newPassword: '', confirmPassword: '' })
        } catch (err) {
            console.error(err)
            const errorMsg = err.response?.data?.error || err.response?.data?.message || 'Failed to update password.'
    ...
    const t = {
        ENG: {
            title: "Expert Professional Profile",
            subtitle: "View and manage your agricultural consultant credentials.",
            ...
            totalConsults: "Total Consultations Managed",
        ...
        GUJ: {
            title: "નિષ્ણાત વ્યાવસાયિક પ્રોફાઇલ",
            subtitle: "તમારા કૃષિ પરામર્શક ઓળખપત્રો જુઓ અને સંચાલિત કરો.",
            securityTitle: "એકાઉન્ટ અને પાસવર્ડ સુરક્ષા",
            changePwd: "અહીંથી પાસવર્ડ બદલો",
            currPwd: "વર્તમાન ગુપ્ત કોડ",
            newPwd: "નવો ગુપ્ત કોડ",
            confPwd: "નવા ગુપ્ત કોડની પુષ્ટિ કરો",
            updatePwdBtn: "પાસવર્ડ અપડેટ કરો",
            certInfo: "પ્રમાણપત્ર સ્થિતિ",
            verifiedText: "સરકારી રજિસ્ટર્ડ કન્સલ્ટન્ટ",
            registerDate: "નોંધણી તારીખ",
            totalConsults: "કુલ નિષ્પાદિત કેસો",
    ...
    if (isLoading) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[400px]">
                ...
                <p className="mt-4 text-dark-light text-sm font-semibold">Loading profile...</p>
            </div>
        )
    }
    ...
    if (isError) {
        return (
            <div className="p-6 bg-white rounded-card shadow-sm text-center mt-12 max-w-xl mx-auto border">
                ...
                <h3 className="text-lg font-bold text-dark">Failed to load profile.</h3>
                <Button onClick={fetchProfile} className="mt-4 bg-emerald-600">Retry</Button>
            </div>
        )
    }
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <Card className="lg:col-span-1 p-6 space-y-6">
                    ...
                    <div className="border-t border-slate-100 pt-6 space-y-4">
                        <h3 className="text-xs font-bold text-dark-light uppercase tracking-wider">{t.certInfo}</h3>
                        <div className="flex items-start gap-3 bg-emerald-50/50 p-3 rounded-xl border border-emerald-100 text-xs">
                            ...
                            <div>
                                ...
                                <p className="text-dark-light/80 mt-0.5">Gujarat Dept of Agriculture approved</p>
                            </div>
                        </div>

                        <div className="flex items-center gap-3 text-xs text-dark-light">
                            <FiCalendar size={15} />
                            <span>{t.registerDate}: <strong>{profile.created_date ? new Date(profile.created_date).toLocaleDateString() : 'N/A'}</strong></span>
                        </div>
                        <div className="flex items-center gap-3 text-xs text-dark-light">
                            <FiAward size={15} />
                            <span>{t.totalConsults}: <strong>{profile.total_consultations || 0} Cases</strong></span>
                        </div>
                    </div>
                </Card>
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <Card className="p-6 space-y-5">
                        <div className="flex items-center justify-between pb-2 border-b border-slate-100">
                            <h3 className="font-bold text-slate-800 text-lg flex items-center gap-2">
                                <FiBookOpen className="text-emerald-600" /> {t.profileInfo}
                            </h3>
                            {!isEditing ? (
                                <button
                        ...
                        {isEditing ? (
                            <form onSubmit={handleProfileSave} className="space-y-4">
                                <div className="flex flex-col gap-1.5">
                                    <label className="text-xs font-bold text-slate-500">{t.bioLabel}</label>
                                    <textarea
                                        name="bio"
                                        value={editForm.bio}
                                        ...
                                        placeholder={lang === 'GUJ' ? 'તમારો પ્રોફેશનલ પરિચય લખો...' : 'Write your professional biography...'}
                                    />
                                </div>

                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div className="flex flex-col gap-1.5">
                                        <label className="text-xs font-bold text-slate-500">{t.qualLabel}</label>
                                        <input
                                            name="qualification"
                                    ...
                                    <div className="flex flex-col gap-1.5">
                                        <label className="text-xs font-bold text-slate-500">{t.expLabel}</label>
                                        <input
                                            name="experience"
                                            type="number"
                                            min="0"
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div className="flex flex-col gap-1.5">
                                        <label className="text-xs font-bold text-slate-500">{t.phoneLabel}</label>
                                        <input
                                            name="phone"
                                    ...
                                    <div className="flex flex-col gap-1.5">
                                        <label className="text-xs font-bold text-slate-500">{t.photoLabel}</label>
                                        <input
                                            name="photo"
                            ...
                            <div className="space-y-4 text-sm">
                                <div className="space-y-1">
                                    <p className="text-[10px] text-slate-400 font-bold uppercase">{t.bioLabel}</p>
                                    <p className="text-slate-700 font-medium leading-relaxed">{profile.bio || <span className="italic text-slate-400">Not set</span>}</p>
                                </div>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2 border-t border-slate-50">
                                    <div className="space-y-1">
                                        <p className="text-[10px] text-slate-400 font-bold uppercase">{t.qualLabel}</p>
                                        <p className="text-slate-700 font-semibold">{profile.qualification || 'N/A'}</p>
                                    </div>
                                    <div className="space-y-1">
                                        <p className="text-[10px] text-slate-400 font-bold uppercase">{t.expLabel}</p>
                                        <p className="text-slate-700 font-semibold">{profile.experience} {lang === 'GUJ' ? 'વર્ષ' : 'Years'}</p>
                                    </div>
                                    <div className="space-y-1">
                                        <p className="text-[10px] text-slate-400 font-bold uppercase">{t.phoneLabel}</p>
                                        <p className="text-slate-700 font-semibold">{profile.phone || 'Not set'}</p>
                                    </div>
                                    <div className="space-y-1">
                                        <p className="text-[10px] text-slate-400 font-bold uppercase">{t.photoLabel}</p>
                                        <p className="text-slate-700 font-semibold truncate max-w-[200px]">{profile.photo || 'Not set'}</p>
                                    </div>
                                </div>
                            </div>
                        )}
                    ...
                    <Card className="p-6 space-y-6">
                        <div>
                            ...
                            <p className="text-xs text-dark-light">Keep your account secure by rotating your password regularly.</p>
                        </div>
